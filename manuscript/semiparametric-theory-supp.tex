\section{Semiparametric approaches}
\label{sec:semi-supp}
We first provide master results that
allow semi-parametric estimation of the link functions.  Consider
Lipschitz symmetric link functions, and define for $\lipconst > 0$
\begin{align*}
...
making estimation easier. Recall the distance $d_{\flink}$ in Eq.~\eqref{eqn:link-distance}.
...
\begin{assumption}
  ...
  \poploss(\theta, \vec{\sigma})$ is continuous for $d_{\flink}$ restricted
  to $\R^d \times \fsymlinksp$ at $(u\opt, \vec{\sigma}\opt)$.
\end{assumption}
...
conditions sufficient for it in Lemma~\ref{lem:d-continuity-guarantees}. We can then give the formal version of the master result in Theorem~\ref{theorem:semiparametric-master-informal} as follows.
...
Next we give the promised sufficient conditions allowing the continuity of $\nabla_\theta^2 \poploss(\theta, \vec{\sigma})$ at $(u\opt, \vec{\sigma}\opt)$, including that the collection of links
...
\begin{lemma} \label{lem:d-continuity-guarantees}
  Let $d_{\flink}$ be the distance~\eqref{eqn:link-distance}. Let
  ...
  $d_{\flink}$ restricted to $\R^d \times \fsymlinksp$ at $(u\opt,
  \vec{\sigma}\opt)$ if $\Ep [\ltwo{X}^4] < \infty$ and either of the
  following conditions holds:
  \begin{enumerate}[leftmargin=2em,label=(C.\arabic*)]
  \item For any $\vec{\sigma} = (\sigma_1, \ldots, \sigma_m) \in
    \fsymlinksp$, $\sigma_j'$ are Lipschitz continuous.
